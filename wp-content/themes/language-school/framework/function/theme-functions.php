...
function register_css_styles() {
	if (!is_admin()) {
		...
		if (CMSMASTERS_LEARNPRESS) {
			...
			wp_enqueue_style('theme-cmsmasters-learnpress-style', get_template_directory_uri() . '/css/cmsmasters-learnpress-style.css', $depend_learnpress_style, '1.0.1', 'screen');
			
			wp_enqueue_style('theme-cmsmasters-learnpress-adaptive', get_template_directory_uri() . '/css/cmsmasters-learnpress-adaptive.css', $depend_learnpress_style, '1.0.1', 'screen');
			
			
			if (is_rtl()) {
				wp_enqueue_style('theme-cmsmasters-learnpress-rtl', get_template_directory_uri() . '/css/cmsmasters-learnpress-rtl.css', $depend_learnpress_style, '1.0.1', 'screen');
			}
		}
...
if (CMSMASTERS_LEARNPRESS) {
	function language_school_learnpress_advanced_settings_filter( $settings ) {
		$cmsmasters_option = language_school_get_global_options();

		foreach ( $settings as $index => $setting ) {
			if ( 'primary_color' === $setting['id'] ) {
				$settings[ $index ]['default'] = $cmsmasters_option['language-school' . '_default_link'];
				$settings[ $index ]['desc'] = sprintf( __( 'Default: %s', 'language-school' ), '<code>' . $cmsmasters_option['language-school' . '_default_link'] . '</code>' );
			} elseif ( 'secondary_color' === $setting['id'] ) {
				$settings[ $index ]['default'] = $cmsmasters_option['language-school' . '_default' . '_heading'];
				$settings[ $index ]['desc'] = sprintf( __( 'Default: %s', 'language-school' ), '<code>' . $cmsmasters_option['language-school' . '_default' . '_heading'] . '</code>' );
			}
		}
	...
	add_filter( 'learn_press_advanced_settings', 'language_school_learnpress_advanced_settings_filter' );


	function language_school_learnpress_print_custom_styles() {
		$cmsmasters_option = language_school_get_global_options();
		...
		<style id="learn-press-custom-css">
			:root {
				--lp-primary-color: <?php echo ! empty( $primary_color ) ? $primary_color : $cmsmasters_option['language-school' . '_default_link']; ?>;
				--lp-secondary-color: <?php echo ! empty( $secondary_color ) ? $secondary_color : $cmsmasters_option['language-school' . '_default' . '_heading']; ?>;
			}
		</style>
	...
	add_action( 'wp_head', 'language_school_learnpress_print_custom_styles' );
	
	function language_school_learnpress_remove_custom_meta_box() {
		remove_meta_box( 'cmsmasters_custom_meta_box', 'lp_course' , 'normal' );
		remove_meta_box( 'cmsmasters_custom_meta_box', 'lp_lesson' , 'normal' );
		remove_meta_box( 'cmsmasters_custom_meta_box', 'lp_question' , 'normal' );
	...
	add_action( 'add_meta_boxes' , 'language_school_learnpress_remove_custom_meta_box', 99 );
}
