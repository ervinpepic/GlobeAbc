...
<div class="condition-block customer_choice<?php echo ( $guest_booking || get_option('users_can_register') && !is_user_logged_in() || is_user_logged_in() ? ' default' : '' ); ?>" id="condition-new">
	<form action="" method="post" id="newAppointmentForm">
		...
		// User limit reached
		if ( $reached_limit ) {
			$error_message = sprintf(_n("Sorry, but you've hit the session limit. Each user may only book %d session at a time.","Sorry, but you've hit the session limit. Each user may only book %d sessions at a time.", $appointment_limit, "booked" ), $appointment_limit);
		}
		...
		if ( $is_user_logged_in && ! $error_message ) {
			$msg = sprintf( _n( 'You are about to request an session for %s.', 'You are about to request appointments for %s.', $total_appts, 'booked' ), '<em>' . booked_get_name( $booked_current_user->ID ) . '</em>' ) . ' ' . _n( 'Please review and confirm that you would like to request the following session:', 'Please review and confirm that you would like to request the following sessions:', $total_appts, 'booked' );
			echo wpautop( $msg );
		}

		// If there aren't any errors, and the user isn't logged in
		if ( ! $is_user_logged_in && ! $error_message ) {
			$msg = _n( 'Please confirm that you would like to request the following session:', 'Please confirm that you would like to request the following sessions:', $total_appts, 'booked' );
			echo wpautop( $msg );
		}
		...
		<div class="field">
			...
			<?php else: ?>
				<input type="submit" id="submit-request-appointment" class="button button-primary" value="<?php echo ( $new_appointment_default == 'draft' ? esc_html( _n( 'Request Session', 'Request Session', $total_appts, 'booked' ) ) : esc_html( _n( 'Book Session', 'Book Session', $total_appts, 'booked' ) ) ); ?>">
				<button class="cancel button"><?php esc_html_e('Cancel','booked'); ?></button>
			<?php endif; ?>
		</div>
	</form>
